...
class Processor(nn.Module):
    ...
    def __init__(self, **kwargs):
        ...
        for i in range(kwargs["num_mp_steps"]):
            mp_layer = ProcessorBlock(
                kwargs["node_latent_dim"],
                kwargs["edge_latent_dim"],
                kwargs["mp_type"],
                kwargs["mp_aggr"],
            )
        ...
        if self.mp_type == "GINE":
            self.lin_out = Linear(
                kwargs["node_latent_dim"] * kwargs["num_mp_steps"],
                kwargs["node_latent_dim"],
            )
    ...
    def forward(self, x, edge_index, edge_attr):
        ...
        if self.dropout > 0:
            while True:
                edge_index_, edge_mask = dropout_edge(
                    edge_index, p=self.dropout, training=self.training
                )
...
class ProcessorBlock(nn.Module):
    ...
    def __init__(
        self, node_latent_dim, edge_latent_dim, mp_type="GAT", mp_aggr="softmax"
    ):
        super().__init__()
        ...
        if mp_type == "GAT":
            ...
            gin_mlp = MLP(
                in_channels=node_latent_dim,
                hidden_channels=node_latent_dim * 2,
                out_channels=node_latent_dim,
                num_layers=2,
                act="relu",
                norm="LayerNorm",
            ...
            self.conv = GINEConv(
                gin_mlp, node_latent_dim, node_latent_dim, edge_dim=edge_latent_dim
            )
        else:
